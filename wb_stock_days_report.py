...
def load_inbound(storage: S3Storage):
    ...
    for key in keys:
        ...
        if not m:
            continue
        eta_date = datetime.strptime(m.group(0), "%d-%m-%y").date() + timedelta(days=14)
        ...
        if qty_col is None:
            continue
        temp = df[["CODES", qty_col]].copy()
        temp.columns = ["Артикул 1С", "Товары в пути, шт"]
        temp["Артикул 1С"] = temp["Артикул 1С"].astype(str).str.strip()
        temp["Товары в пути, шт"] = to_numeric(temp["Товары в пути, шт"]).map(round_int)
        temp = temp[(temp["Артикул 1С"] != "") & (temp["Артикул 1С"].str.lower() != "nan") & (temp["Товары в пути, шт"] > 0)]
        if temp.empty:
            continue
        temp["Ближайшее поступление"] = eta_date
        frames.append(temp)

    if not frames:
        return pd.DataFrame(columns=["Артикул 1С", "Товары в пути, шт", "Ближайшее поступление", "Дней до поступления"])

    inbound = pd.concat(frames, ignore_index=True)
    total = inbound.groupby("Артикул 1С", as_index=False)["Товары в пути, шт"].sum()
    nearest = inbound.groupby("Артикул 1С", as_index=False)["Ближайшее поступление"].min()
    res = total.merge(nearest, on="Артикул 1С", how="left")
    today = datetime.now().date()
    res["Дней до поступления"] = res["Ближайшее поступление"].map(lambda d: max((d - today).days, 0) if pd.notna(d) else 9999)
    return res
...
def load_latest_wb_stocks(storage: S3Storage):
    ...
    df["_sample_dt"] = pd.to_datetime(df[dt_col], errors="coerce")
    latest_dt = df["_sample_dt"].max()
    df = df[df["_sample_dt"] == latest_dt].copy()

    wb_col = choose_existing_column(df, ["Артикул WB", "nmId"], "Артикул WB")
    supplier_col = choose_existing_column(df, ["Артикул продавца"], "Артикул продавца")
    qty_col = choose_existing_column(df, ["Доступно для продажи", "Полное количество", "Остатки", "Остаток", "Количество"], "остаток WB")

    temp = df[[wb_col, supplier_col, qty_col]].copy()
    temp.columns = ["Артикул WB", "Артикул WB продавца", "Остаток WB, шт"]
    temp["Артикул WB"] = temp["Артикул WB"].map(normalize_str)
    temp["Артикул WB продавца"] = temp["Артикул WB продавца"].astype(str).str.strip()
...
def build_sales_by_windows(orders: pd.DataFrame):
    empty_cols = ["Артикул WB", "Артикул WB продавца", "Продажи 7 дней, шт", "Продажи 60 дней, шт", "Среднесуточные продажи 7д", "Среднесуточные продажи 60д", "Цена покупателя"]
    if orders.empty:
        return pd.DataFrame(columns=empty_cols)

    wb_col = choose_existing_column(orders, ["nmId", "Артикул WB"], "Артикул WB в заказах")
    supplier_col = choose_existing_column(orders, ["supplierArticle", "Артикул продавца"], "Артикул продавца в заказах")
    date_col = choose_existing_column(orders, ["date", "Дата", "Дата заказа", "lastChangeDate", "Дата продажи"], "дата в заказах")

    work = orders.copy()
    work["Артикул WB"] = work[wb_col].map(normalize_str)
    work["Артикул WB продавца"] = work[supplier_col].astype(str).str.strip()
    ...
    keys = ["Артикул WB", "Артикул WB продавца"]
    s7 = work[work["dt"] >= start_7].groupby(keys).size().rename("Продажи 7 дней, шт")
    s60 = work[work["dt"] >= start_60].groupby(keys).size().rename("Продажи 60 дней, шт")
    ...
    if "finishedPrice" in work.columns:
        price_last = work[work["dt"] == max_dt].groupby(keys)["finishedPrice"].mean().rename("Цена покупателя").reset_index()
        price_last["Цена покупателя"] = to_numeric(price_last["Цена покупателя"]).map(round_int)
        res = res.merge(price_last, on=keys, how="left")
    else:
...
def load_current_month_zero_days(storage: S3Storage, zero_wb_articles: set):
    if not zero_wb_articles:
        return {}
    ...
    for key in sorted(storage.list_keys(WB_STOCKS_PREFIX), key=parse_week_from_key):
        ...
        wb_col = choose_existing_column(df, ["Артикул WB", "nmId"], "Артикул WB в остатках месяца")
        qty_col = choose_existing_column(df, ["Доступно для продажи", "Полное количество"], "остаток WB в остатках месяца")
        dt_col = choose_existing_column(df, ["Дата сбора", "Дата запроса"], "дата среза остатков")

        temp = df[[wb_col, qty_col, dt_col]].copy()
        temp.columns = ["Артикул WB", "stock_wb", "sample_dt"]
        temp["Артикул WB"] = temp["Артикул WB"].map(normalize_str)
        temp["stock_wb"] = to_numeric(temp["stock_wb"])
        temp["sample_dt"] = pd.to_datetime(temp["sample_dt"], errors="coerce").dt.normalize()
        temp = temp[(temp["Артикул WB"].isin(zero_wb_articles)) & temp["sample_dt"].notna()]
    ...
    month_df = month_df.groupby(["Артикул WB", "sample_dt"], as_index=False)["stock_wb"].sum()
    month_df["is_zero"] = month_df["stock_wb"] <= 0
    out = month_df.groupby("Артикул WB")["is_zero"].sum().to_dict()
...
def build_report_dataframe(wb_stocks, sales, article_map, stocks_1c, stop_articles, rrc_df, inbound_df, zero_days_map):
    ...
    df["Артикул 1С"] = df["Артикул 1С"].fillna("").astype(str).str.strip()
    ...
    df["Товары в пути, шт"] = pd.to_numeric(df.get("Товары в пути, шт", 0), errors="coerce").fillna(0).map(round_int)
    df["Дней до поступления"] = pd.to_numeric(df.get("Дней до поступления", 9999), errors="coerce").fillna(9999).map(round_int)

    df["Расчётный спрос в день, шт"] = df.apply(select_daily_sales, axis=1)
    df["WB хватит, дней"] = df.apply(lambda r: safe_div(r["Остаток WB, шт"], r["Расчётный спрос в день, шт"]), axis=1)
    df["Липецк хватит, дней"] = df.apply(lambda r: safe_div(r["Остатки МП (Липецк), шт"], r["Расчётный спрос в день, шт"]), axis=1)
    df["WB + Липецк, дней"] = df.apply(lambda r: safe_div(r["Остаток WB, шт"] + r["Остатки МП (Липецк), шт"], r["Расчётный спрос в день, шт"]), axis=1)
    df["WB + Липецк + в пути, дней"] = df.apply(lambda r: safe_div(r["Остаток WB, шт"] + r["Остатки МП (Липецк), шт"] + r["Товары в пути, шт"], r["Расчётный спрос в день, шт"]), axis=1)

    df["Хватит до поступления"] = df.apply(lambda r: "OK" if r["WB + Липецк, дней"] >= r["Дней до поступления"] else "Нет", axis=1)
    df["Остатки кончатся через, дней"] = df["WB + Липецк, дней"].clip(lower=0)
    df["Аут оф сток до поступления, дней"] = df.apply(lambda r: max(r["Дней до поступления"] - r["WB + Липецк, дней"], 0), axis=1)
    df["0 остаток на горизонте 70 дней"] = df.apply(lambda r: max(70 - r["WB + Липецк + в пути, дней"], 0), axis=1)
    ...
    if "Ближайшее поступление" in df.columns:
        df["Ближайшее поступление"] = pd.to_datetime(df["Ближайшее поступление"], errors="coerce").dt.date

    for col in ["Среднесуточные продажи 7д", "Среднесуточные продажи 60д", "Расчётный спрос в день, шт", "WB хватит, дней", "Липецк хватит, дней", "WB + Липецк, дней", "WB + Липецк + в пути, дней", "Остатки кончатся через, дней", "Аут оф сток до поступления, дней", "0 остаток на горизонте 70 дней"]:
        df[col] = pd.to_numeric(df[col], errors="coerce").fillna(0).map(round_int)

    result_cols = ["Артикул 1С", "Артикул WB", "Артикул WB продавца", "Остаток WB, шт", "Остатки МП (Липецк), шт", "Товары в пути, шт", "Ближайшее поступление", "Дней до поступления", "Продажи 7 дней, шт", "Продажи 60 дней, шт", "Среднесуточные продажи 7д", "Среднесуточные продажи 60д", "Расчётный спрос в день, шт", "WB хватит, дней", "Липецк хватит, дней", "WB + Липецк, дней", "WB + Липецк + в пути, дней", "Хватит до поступления", "Остатки кончатся через, дней", "Аут оф сток до поступления, дней", "0 остаток на горизонте 70 дней", "Дней без остатка WB в текущем месяце", "Цена покупателя", "РРЦ", "Коэффициент", "Delist"]
    df = df[result_cols].copy()

    agg = {"Артикул WB": "first", "Артикул WB продавца": "first", "Остаток WB, шт": "sum", "Остатки МП (Липецк), шт": "max", "Товары в пути, шт": "max", "Ближайшее поступление": "min", "Дней до поступления": "min", "Продажи 7 дней, шт": "sum", "Продажи 60 дней, шт": "sum", "Среднесуточные продажи 7д": "max", "Среднесуточные продажи 60д": "max", "Расчётный спрос в день, шт": "max", "WB хватит, дней": "max", "Липецк хватит, дней": "max", "WB + Липецк, дней": "max", "WB + Липецк + в пути, дней": "max", "Хватит до поступления": "first", "Остатки кончатся через, дней": "max", "Аут оф сток до поступления, дней": "max", "0 остаток на горизонте 70 дней": "max", "Дней без остатка WB в текущем месяце": "max", "Цена покупателя": "max", "РРЦ": "max", "Коэффициент": "first", "Delist": "first"}
...
def split_report_sheets(df):
    critical = df[(df["Продажи 60 дней, шт"] > 0) & ((df["Остаток WB, шт"] <= 0) | (df["WB хватит, дней"] < 14) | (df["Хватит до поступления"] == "Нет"))].copy()
    critical["Комментарий"] = critical.apply(lambda r: f"Остатки кончатся через {r['Остатки кончатся через, дней']} дн.; прогноз аут оф сток {r['Аут оф сток до поступления, дней']} дн." if r["Хватит до поступления"] == "Нет" else "", axis=1)
    calc = df.copy()
    dead = df[df["WB + Липецк + в пути, дней"] > 120].copy()
    monitor = df.copy()
    critical = critical[["Артикул 1С", "WB хватит, дней", "Липецк хватит, дней", "WB + Липецк, дней", "Товары в пути, шт", "Ближайшее поступление", "Дней до поступления", "Остатки кончатся через, дней", "Аут оф сток до поступления, дней", "Остаток WB, шт", "Остатки МП (Липецк), шт", "Продажи 60 дней, шт", "Дней без остатка WB в текущем месяце", "Комментарий", "Delist"]].copy()
    dead = dead[["Артикул 1С", "WB хватит, дней", "Липецк хватит, дней", "WB + Липецк, дней", "WB + Липецк + в пути, дней", "Остаток WB, шт", "Остатки МП (Липецк), шт", "Товары в пути, шт", "Продажи 60 дней, шт", "Цена покупателя", "РРЦ", "Коэффициент", "Delist"]].copy()
    monitor = monitor[["Артикул 1С", "Остаток WB, шт", "Остатки МП (Липецк), шт", "Товары в пути, шт", "Ближайшее поступление", "Дней до поступления", "WB + Липецк, дней", "WB + Липецк + в пути, дней", "Хватит до поступления", "Аут оф сток до поступления, дней", "0 остаток на горизонте 70 дней", "Продажи 60 дней, шт", "Delist"]].copy()
...
def auto_fit_ws(ws):
    ...
    for col_idx, width in widths.items():
        ws.column_dimensions[get_column_letter(col_idx)].width = min(max(width + 2, 14), 38)


def style_ws(ws, sheet_type="normal", price_cols=None, black_col=None):
    ...
    if sheet_type == "monitor":
        header = [c.value for c in ws[1]]
        if "WB + Липецк + в пути, дней" in header:
            idx = header.index("WB + Липецк + в пути, дней") + 1
            for r in range(2, ws.max_row + 1):
                cell = ws.cell(r, idx)
                try:
                    val = float(cell.value)
...
def save_report_xlsx(critical_df, calc_df, dead_df, monitor_df, out_path: str):
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
        critical_df.to_excel(writer, sheet_name=SHEET_CRITICAL, index=False)
        calc_df.to_excel(writer, sheet_name=SHEET_CALC, index=False)
        dead_df.to_excel(writer, sheet_name=SHEET_DEAD, index=False)
        monitor_df.to_excel(writer, sheet_name=SHEET_MONITOR, index=False)
    ...
    price_cols = {dead_header.index(n) + 1 for n in ["Цена покупателя", "РРЦ", "Коэффициент"] if n in dead_header}
    black_col = dead_header.index("WB + Липецк + в пути, дней") + 1 if "WB + Липецк + в пути, дней" in dead_header else None
    style_ws(ws_dead, price_cols=price_cols, black_col=black_col)
    style_ws(ws_monitor, sheet_type="monitor")
    wb.save(out_path)
...
def send_to_telegram(cfg: Config, file_path: str, critical_count: int, dead_count: int):
    ...
    with open(file_path, "rb") as f:
        resp = requests.post(url, data={"chat_id": cfg.telegram_chat_id, "caption": caption}, files={"document": (os.path.basename(file_path), f)}, timeout=120)
...
def run():
    ...
    inbound_df = load_inbound(storage)
    current_zero_articles = set(wb_stocks.loc[wb_stocks["Остаток WB, шт"] <= 0, "Артикул WB"].tolist())
    zero_days_map = load_current_month_zero_days(storage, current_zero_articles)
    report_df = build_report_dataframe(wb_stocks, sales_df, article_map, stocks_1c, stop_articles, rrc_df, inbound_df, zero_days_map)
    critical_df, calc_df, dead_df, monitor_df = split_report_sheets(report_df)
    out_path = os.path.join(OUT_DIR, f"Отчёт_дни_остатка_WB_{STORE_NAME}_{datetime.now().strftime('%Y%m%d')}.xlsx")
    save_report_xlsx(critical_df, calc_df, dead_df, monitor_df, out_path)
