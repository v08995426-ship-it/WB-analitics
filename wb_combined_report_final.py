...
class CombinedReport:
    ...
    def build_log_report(self) -> Workbook:
        """Build a slim diagnostics workbook.

        Full raw-source tabs made the workflow extremely slow and could exhaust
        memory on GitHub Actions. Keep only diagnostics and compact previews.
        """
        writer = ReportWriter()
        log("Building slim log workbook")
        writer.add_df_sheet("source_summary", self.source_summary)
        writer.add_df_sheet("sku_master", self.sku_master, limit=5000)
        writer.add_df_sheet("daily_current", self.daily_current, limit=10000)
        writer.add_df_sheet("daily_targets", self.daily_targets, limit=10000)
        writer.add_df_sheet("weekly_summary", self.weekly_summary, limit=10000)
        writer.add_df_sheet("monthly_summary", self.monthly_summary, limit=5000)
        writer.add_df_sheet("monthly_forecast", self.monthly_forecast, limit=5000)

        preview_rows = [
            ("orders_preview", self.data.orders),
            ("stocks_preview", self.data.stocks),
            ("search_preview", self.data.search),
            ("search_history_preview", self.data.search_history),
            ("funnel_preview", self.data.funnel),
            ("ads_daily_preview", self.data.ads_daily),
            ("ads_total_preview", self.data.ads_total),
            ("campaigns_preview", self.data.campaigns),
            ("economics_preview", self.data.economics),
            ("abc_preview", self.data.abc),
            ("entry_points_category_preview", self.data.entry_points_category),
            ("entry_points_sku_preview", self.data.entry_points_sku),
            ("stock_history_days_preview", self.data.stock_history_days),
            ("stock_history_detail_preview", self.data.stock_history_detail),
        ]
        source_meta = []
        for name, df in preview_rows:
            source_meta.append({
                "sheet": name,
                "rows_total": 0 if df is None else len(df),
                "cols_total": 0 if df is None else len(df.columns),
                "rows_saved": 0 if df is None else min(len(df), 500),
            })
            writer.add_df_sheet(name, df, limit=500)
        writer.add_df_sheet("source_meta", pd.DataFrame(source_meta))
        ...
        if self.data.diagnostics:
            ...
            writer.add_df_sheet("diagnostics", pd.DataFrame(diag_rows), limit=5000)
        else:
...
def main() -> int:
    ...
    log("Building main workbook")
    main_wb = report.build_main_report()
    log("Built main workbook")
    log_wb = report.build_log_report()
    log("Built log workbook")
