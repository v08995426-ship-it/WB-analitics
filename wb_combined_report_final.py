...
class Builder:
    ...
    def pick_econ_for_day(self, daily_keys: pd.DataFrame) -> Tuple[pd.DataFrame,pd.DataFrame]:
        ...
        for rec in daily_keys.itertuples(index=False):
            ...
            if g.empty and pd.notna(nm): g=self.econ[self.econ["nm_id"]==nm].copy()
            if g.empty:
                missing += 1
                diag.append({"day":day,"supplier_article":art,"nm_id":nm,"target_week":target_week,"picked_week":None,"match_type":"missing"})
                continue
            eg=g[g["week"].astype(str)==str(target_week)]
            if not eg.empty:
                chosen=eg.iloc[0]; exact += 1; mt="exact"
            else:
                # fallback latest non-null commission first, then latest
                g2=g.sort_values(["commission_pct","week_start"], ascending=[False,False])
                chosen=g2.iloc[0]; fallback += 1; mt="fallback_latest"
            ...
            rows.append(row)
            diag.append({"day":day,"supplier_article":art,"nm_id":nm,"target_week":target_week,"picked_week":chosen.get("week"),"match_type":mt})
    ...
    def build_daily_calc(self) -> Tuple[pd.DataFrame,Dict[str,pd.DataFrame]]:
        ...
        log(f"Commission diagnostics: zero/empty commission_pct rows = {(daily['commission_pct'].fillna(0)==0).sum():,} из {len(daily):,}")
        tech={
