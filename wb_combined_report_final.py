...
class DataLoader:
    ...
    def load_orders(self) -> pd.DataFrame:
        ...
        dfs = []
        usecols = lambda c: norm_key(c) in {norm_key(x) for x in [
            "date", "Дата", "Дата заказа", "warehouseName", "Склад", "supplierArticle", "Артикул продавца",
            "nmId", "Артикул WB", "subject", "Предмет", "brand", "Бренд", "finishedPrice",
            "priceWithDisc", "spp", "isCancel"
        ]}
        needed = ["date", "Дата", "Дата заказа", "warehouseName", "Склад", "supplierArticle", "Артикул продавца", "nmId", "Артикул WB", "subject", "Предмет", "brand", "Бренд", "finishedPrice", "priceWithDisc", "spp", "isCancel"]
        for path in files:
            try:
                df = pd.DataFrame()
                read_errors = []
                for attempt in (
                    lambda: self._read_selected_fast(path, sheet_name=0, header_row=1, needed_headers=needed),
                    lambda: self._read_selected(path, sheet_name=0, header=0, usecols=usecols),
                    lambda: self._read_df(path, ["Заказы"], (0, 1, 2)),
                    lambda: self._read_df(path, None, (0, 1, 2)),
                ):
                    try:
                        candidate = attempt()
                ...
                if df.empty:
                    self.warnings.append(f"Orders read skipped {path}: пустой результат; attempts={'; '.join(read_errors[:3])}")
                    continue
                ...
                if "orders" not in df.columns:
                    ...
                    df["orders"] = to_numeric(df["orders"]).fillna(1)
        ...
        if not out.empty and "day" in out.columns:
