...
def подготовить_финансовые_строки(fin_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # База выручки и база для налогов
    df["signed_revenue"] = df["retail_price_withdisc_rub"] * df["sign"]

    # Цена, которую заплатил покупатель, оставляем отдельной аналитикой
...
def анализ_складов(logistic_rows: pd.DataFrame, начало_недели: datetime.date) -> pd.DataFrame:
    if logistic_rows.empty:
        return pd.DataFrame(columns=[
            "Неделя", "Склад", "Средний коэффициент", "Количество продаж, шт",
            "Логистика факт", "Логистика при коэффициенте 1,4", "Переплата", "Переплата на единицу"
        ])
...
def построить_анализ_неделя_к_неделе(current_fact: pd.DataFrame, prev_fact: pd.DataFrame) -> pd.DataFrame:
    ...
    merge_cols = [
        "Артикул WB", "Артикул продавца", "Предмет", "Чистая прибыль", "Валовая прибыль",
        "Валовая выручка", "Реклама", "Комиссия WB", "Логистика прямая", "Логистика обратная",
        "СПП, %", "Средняя цена продажи", "Средняя цена покупателя", "Продажи, шт", "Процент выкупа"
    ]
