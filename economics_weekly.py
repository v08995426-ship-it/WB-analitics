...
def прочитать_себестоимость(s3: S3Storage) -> pd.DataFrame:
    ...
    return нормализовать_себестоимость(raw)
...
def подготовить_рекламный_лист(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = привести_к_числам(
        df,
        ["Артикул WB", "Расход", "Сумма заказов", "Показы", "Клики", "Заказы"]
    )
    df["Артикул WB"] = pd.to_numeric(df["Артикул WB"], errors="coerce")
...
def прочитать_рекламу_недели(s3: S3Storage, начало_недели: datetime.date, конец_недели: datetime.date) -> pd.DataFrame:
    try:
        ...
        if not df.empty:
            return df[(df["Дата"] >= начало_недели) & (df["Дата"] <= конец_недели)].copy()
        else:
            лог("⚠️ В Анализ рекламы.xlsx не найден лист с колонками рекламы")
    ...
    if not s3.file_exists(weekly_key):
        лог(f"⚠️ Не найден weekly-рекламный файл: {weekly_key}")
        return pd.DataFrame()
    ...
    try:
        ...
        if not df.empty:
            ...
            лог("⚠️ В weekly-рекламном файле не найден лист с колонками рекламы")
    except Exception as e:
...
def анализ_складов(logistic_rows: pd.DataFrame, начало_недели: datetime.date) -> pd.DataFrame:
    ...
    tmp = df.apply(
        lambda r: pd.Series(
            пересчёт_строки(r),
            index=["Логистика факт", "Логистика при коэффициенте 1,4", "Переплата"]
        ),
        axis=1
    )
    ...
    return out[[
        ...
        "Переплата",
        "Переплата на единицу",
    ]].sort_values("Переплата", ascending=False)
...
def посчитать_факт_и_юнит(
    ...
    средняя_цена_продажи = (
        валовая_выручка / чистые_продажи.replace(0, pd.NA)
    ).rename("Средняя цена продажи")
    средняя_цена_покупателя = (
        цена_покупателя_сумма / чистые_продажи.replace(0, pd.NA)
    ).rename("Средняя цена покупателя")
    ...
    total_storage_store = abs(pd.to_numeric(df.get("storage_fee", 0), errors="coerce").fillna(0).sum())
    total_acceptance_store = abs(pd.to_numeric(df.get("acceptance", 0), errors="coerce").fillna(0).sum())
    total_penalty_store = abs(pd.to_numeric(df.get("penalty", 0), errors="coerce").fillna(0).sum())
    total_deduction_store = abs(pd.to_numeric(df.get("deduction", 0), errors="coerce").fillna(0).sum())
    ...
    storage_alloc = распределить_хранение(stocks_df, total_storage_store, начало_недели, конец_недели)
    base = base.merge(storage_alloc, on="Артикул WB", how="left")
    if "Хранение" not in base.columns:
        base["Хранение"] = 0.0
    base["Хранение"] = pd.to_numeric(base["Хранение"], errors="coerce").fillna(0)
    ...
    штрафы_на_ед = безопасное_деление(total_penalty_store, total_units_sold_store, 6)
    удержания_на_ед = безопасное_деление(total_deduction_store, total_units_sold_store, 6)

    base["Приёмка"] = base["Продажи, шт"] * приемка_на_ед
    base["Штрафы"] = base["Продажи, шт"] * штрафы_на_ед
    base["Удержания"] = base["Продажи, шт"] * удержания_на_ед
    ...
    unit["Штрафы и удержания, руб/ед"] = unit.apply(lambda r: безопасное_деление(r["Штрафы"] + r["Удержания"], r["Чистые продажи, шт"], 6), axis=1)
...
def объяснение_sku(row) -> str:
    ...
    if not reasons:
        if delta_profit >= 0:
            return "Положительная динамика без одного явного драйвера."
        return "Негативная динамика без одного явного драйвера."
...
def построить_анализ_неделя_к_неделе(current_fact: pd.DataFrame, prev_fact: pd.DataFrame) -> pd.DataFrame:
    ...
    merged["Δ Чистая прибыль"] = merged["Чистая прибыль_тек"] - merged["Чистая прибыль_пред"]
    merged["Δ Валовая выручка"] = merged["Валовая выручка_тек"] - merged["Валовая выручка_пред"]
    merged["Δ Реклама"] = merged["Реклама_тек"] - merged["Реклама_пред"]
    merged["Δ Комиссия WB"] = merged["Комиссия WB_тек"] - merged["Комиссия WB_пред"]
    merged["Δ Логистика"] = (
        (merged["Логистика прямая_тек"] + merged["Логистика обратная_тек"]) -
        (merged["Логистика прямая_пред"] + merged["Логистика обратная_пред"])
    )
    merged["Δ СПП"] = merged["СПП, %_тек"] - merged["СПП, %_пред"]
    merged["Δ вашей цены"] = merged["Средняя цена продажи_тек"] - merged["Средняя цена продажи_пред"]
    merged["Комментарий"] = merged.apply(объяснение_sku, axis=1)

    лидеры_роста = merged.sort_values("Δ Чистая прибыль", ascending=False).head(20)
    лидеры_падения = merged.sort_values("Δ Чистая прибыль", ascending=True).head(20)

    for _, r in лидеры_роста.iterrows():
        rows.append({
            "Раздел": "Лидеры роста",
            "Неделя": current_week,
            "Артикул WB": int(r["Артикул WB"]) if pd.notna(r["Артикул WB"]) else "",
            "Артикул продавца": r.get("Артикул продавца_тек") or r.get("Артикул продавца_пред") or "",
            "Предмет": r.get("Предмет_тек") or r.get("Предмет_пред") or "",
            ...
            "Изменение чистой прибыли": округлить(r["Δ Чистая прибыль"], 2),
            "Изменение выручки": округлить(r["Δ Валовая выручка"], 2),
            "Изменение рекламы": округлить(r["Δ Реклама"], 2),
            "Изменение комиссии": округлить(r["Δ Комиссия WB"], 2),
            "Изменение логистики": округлить(r["Δ Логистика"], 2),
            "Изменение СПП": округлить(r["Δ СПП"], 2),
            "Изменение вашей цены": округлить(r["Δ вашей цены"], 2),
...
def загрузить_существующую_экономику(s3: S3Storage) -> Dict[str, pd.DataFrame]:
    if not s3.file_exists(ПУТЬ_ЭКОНОМИКА):
        return {
            ЛИСТ_ЮНИТ: pd.DataFrame(),
            ЛИСТ_ФАКТ: pd.DataFrame(),
            ЛИСТ_АНАЛИЗ: pd.DataFrame(),
            ЛИСТ_СКЛАДЫ: pd.DataFrame(),
        }

    try:
        sheets = s3.read_excel_all_sheets(ПУТЬ_ЭКОНОМИКА)
        return {
            ЛИСТ_ЮНИТ: sheets.get(ЛИСТ_ЮНИТ, pd.DataFrame()),
            ЛИСТ_ФАКТ: sheets.get(ЛИСТ_ФАКТ, pd.DataFrame()),
            ЛИСТ_АНАЛИЗ: sheets.get(ЛИСТ_АНАЛИЗ, pd.DataFrame()),
            ЛИСТ_СКЛАДЫ: sheets.get(ЛИСТ_СКЛАДЫ, pd.DataFrame()),
        }
    except Exception as e:
    ...
    def run(self):
        ...
        sheets_to_write = {
            ЛИСТ_ЮНИТ: округлить_таблицу(
                юнит_all.sort_values(["Неделя", "Чистая прибыль, руб/ед"], ascending=[True, False]).reset_index(drop=True)
            ),
            ЛИСТ_ФАКТ: округлить_таблицу(
                факт_all.sort_values(["Неделя", "Чистая прибыль"], ascending=[True, False]).reset_index(drop=True)
            ),
            ЛИСТ_АНАЛИЗ: округлить_таблицу(анализ_all.reset_index(drop=True)),
            ЛИСТ_СКЛАДЫ: округлить_таблицу(
                склады_all.sort_values(["Неделя", "Переплата"], ascending=[True, False]).reset_index(drop=True)
            ),
        }
