...
class WBBidSender:
    ...
    WB_MIN_BIDS_WINDOW_SEC = 3.0
    WB_MIN_BIDS_WINDOW_LIMIT = 5
    WB_MIN_BIDS_MINUTE_SEC = 60.0
    WB_MIN_BIDS_MINUTE_LIMIT = 20

    WB_PATCH_SLEEP_SEC = 0.25
    WB_RETRYABLE_HTTP_CODES = {429, 500, 502, 503, 504}

    def __init__(
        self,
        api_key: str,
        metrics_df: pd.DataFrame,
        fallback_bid_step_kopecks: int = 100,
        max_retry_rounds: int = 10,
    ):
        self.api_key = api_key.strip()
        self.metrics_df = metrics_df.copy()
        self.fallback_bid_step_kopecks = int(fallback_bid_step_kopecks)
        self.max_retry_rounds = int(max_retry_rounds)
        ...
        self._min_bids_call_times: List[float] = []
        self._min_bids_cache: Dict[Tuple[int, str, str, Tuple[int, ...]], Dict[Tuple[int, str], int]] = {}

    @staticmethod
    def detect_wb_placement(row: pd.Series) -> str:
        bid_type = str(row.get("Тип ставки", "")).strip().lower()
    ...
    def normalize_bid_for_wb(
        ...
        known_max_bid: Optional[int] = None,
    ) -> int:
        bid_value = int(bid_value)

        if placement == "combined":
            bid_value = max(bid_value, 8000)
        elif placement == "recommendations":
            bid_value = max(bid_value, 8000)
        elif placement == "search":
            bid_value = max(bid_value, 400)

        if known_min_bid is not None and known_min_bid > 0:
            bid_value = max(bid_value, int(known_min_bid))

        if known_max_bid is not None and known_max_bid > 0:
            bid_value = min(bid_value, int(known_max_bid))

        return int(bid_value)

    @staticmethod
    def is_wrong_bid_value_error(response_text: str) -> bool:
        return "wrong bid value" in str(response_text).lower()
    ...
    @staticmethod
    def extract_max_bid_from_error(response_text: str) -> Optional[int]:
        m = re.search(r"max:\s*(\d+)", str(response_text).lower())
        return int(m.group(1)) if m else None
    ...
    def _rate_limit_min_bids(self):
        now = time.time()

        self._min_bids_call_times = [
            t for t in self._min_bids_call_times
            if now - t < self.WB_MIN_BIDS_MINUTE_SEC
        ]

        if len(self._min_bids_call_times) >= self.WB_MIN_BIDS_MINUTE_LIMIT:
            sleep_for = self.WB_MIN_BIDS_MINUTE_SEC - (now - self._min_bids_call_times[0]) + 0.05
            if sleep_for > 0:
                log(f"⏳ bids/min: достигнут лимит 20 запросов/мин, пауза {sleep_for:.2f} сек")
                time.sleep(sleep_for)

        now = time.time()
        window_calls = [
            t for t in self._min_bids_call_times
            if now - t < self.WB_MIN_BIDS_WINDOW_SEC
        ]
        if len(window_calls) >= self.WB_MIN_BIDS_WINDOW_LIMIT:
            sleep_for = self.WB_MIN_BIDS_WINDOW_SEC - (now - window_calls[0]) + 0.05
            if sleep_for > 0:
                log(f"⏳ bids/min: достигнут burst-лимит, пауза {sleep_for:.2f} сек")
                time.sleep(sleep_for)

        self._min_bids_call_times.append(time.time())

    def decisions_to_payload(self, decisions: List[Decision]) -> Dict[str, Any]:
        metrics_lookup = self.metrics_df.set_index(["ID кампании", "Артикул WB"], drop=False)
        ...
        for d in decisions:
            ...
            placement = self.detect_wb_placement(row)
            ...
            grouped[d.id_campaign]["nm_bids"].append({
                "nm_id": int(d.nm_id),
                "bid_kopecks": self.normalize_bid_for_wb(int(bid_value), placement),
                "placement": placement,
            })

        return {"bids": list(grouped.values())}
    ...
    def fetch_min_bids_for_advert(
        ...
        nm_ids_sorted = tuple(sorted(int(x) for x in nm_ids if int(x) > 0))
        placements_sorted = tuple(sorted(str(x).strip().lower() for x in placements if str(x).strip()))
        cache_key = (int(advert_id), payment_type, "|".join(placements_sorted), nm_ids_sorted)

        if cache_key in self._min_bids_cache:
            return self._min_bids_cache[cache_key]

        self._rate_limit_min_bids()

        req_payload = {
            "advert_id": int(advert_id),
            "nm_ids": list(nm_ids_sorted),
            "payment_type": payment_type,
            "placement_types": [self.placement_for_min_bids_api(p) for p in placements_sorted],
        }

        resp = requests.post(
            self.WB_BIDS_MIN_URL,
        ...
        self._min_bids_cache[cache_key] = result
        return result

    def enrich_single_advert_with_min_bids(
        self,
        advert_block: Dict[str, Any],
    ) -> Tuple[Dict[str, Any], pd.DataFrame]:
        advert_id = safe_int(advert_block.get("advert_id"))
        nm_bids = advert_block.get("nm_bids", [])

        if not advert_id or not nm_bids:
            return advert_block, pd.DataFrame()

        nm_ids = [safe_int(x.get("nm_id")) for x in nm_bids if safe_int(x.get("nm_id")) > 0]
        placements = list(sorted(set(
            str(x.get("placement", "")).strip().lower()
            for x in nm_bids if x.get("placement")
        )))
        payment_type = self.extract_payment_type_for_advert(self.metrics_df, advert_id)

        min_map: Dict[Tuple[int, str], int] = {}
        min_fetch_status = "ok"

        try:
            min_map = self.fetch_min_bids_for_advert(
                advert_id=advert_id,
                nm_ids=nm_ids,
                payment_type=payment_type,
            ...
            if status_code == 429:
                min_fetch_status = "rate_limited_429"
                log(f"⚠️ bids/min 429 для advert_id={advert_id}, продолжаю без prefetch min bids")
            else:
                min_fetch_status = f"http_error_{status_code}"
                log(f"⚠️ Ошибка получения min bids advert_id={advert_id}: {e}")
        except Exception as e:
        ...
        for nm_bid in nm_bids:
            ...
            new_bid = self.normalize_bid_for_wb(
                bid_value=old_bid,
                placement=placement,
                known_min_bid=known_min,
                known_max_bid=None,
            )
    ...
    def apply_error_constraints_to_single_advert(
        self,
        advert_payload: Dict[str, Any],
        response_text: str,
    ) -> Dict[str, Any]:
        min_bid = self.extract_min_bid_from_error(response_text)
        max_bid = self.extract_max_bid_from_error(response_text)

        new_nm_bids = []

        for nm_bid in advert_payload.get("nm_bids", []):
            placement = str(nm_bid.get("placement", "")).strip().lower()
            bid_kopecks = safe_int(nm_bid.get("bid_kopecks"))

            corrected = self.normalize_bid_for_wb(
                bid_value=bid_kopecks,
                placement=placement,
                known_min_bid=min_bid,
                known_max_bid=max_bid,
            )

            new_nm_bids.append({
                "nm_id": safe_int(nm_bid.get("nm_id")),
                "bid_kopecks": corrected,
                "placement": placement,
            })

        return {
            "advert_id": safe_int(advert_payload.get("advert_id")),
            "nm_bids": new_nm_bids,
        }

    def bump_single_advert_bids(
        self,
        advert_payload: Dict[str, Any],
        step_kopecks: Optional[int] = None,
    ) -> Dict[str, Any]:
        if step_kopecks is None:
            step_kopecks = self.fallback_bid_step_kopecks
        ...
        for nm_bid in advert_payload.get("nm_bids", []):
            ...
            bumped = self.normalize_bid_for_wb(
                bid_value=bumped,
                placement=placement,
                known_min_bid=None,
                known_max_bid=None,
    ...
    def send_one_advert(
        ...
        advert_payload, min_log_df = self.enrich_single_advert_with_min_bids(advert_payload)
        ...
        for attempt in range(1, self.max_retry_rounds + 1):
            try:
                resp = requests.patch(
                    self.WB_BIDS_URL,
                    headers=self.headers,
                    json={"bids": [advert_payload]},
                    timeout=120
                )

                if resp.status_code == 200:
                    log(f"✅ advert_id={advert_payload['advert_id']} успешно применён с попытки {attempt}")
                    send_log_rows.append({
                        "Дата": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                        "ID кампании": advert_payload["advert_id"],
                        "Попытка": attempt,
                        "Статус": "success",
                        "HTTP статус": 200,
                        "Ответ WB": resp.text[:2000],
                    })
                    time.sleep(self.WB_PATCH_SLEEP_SEC)
                    return True, pd.DataFrame(send_log_rows)

                response_text = resp.text[:3000]
                log(f"⚠️ advert_id={advert_payload['advert_id']} ошибка WB {resp.status_code}: {response_text}")
                ...
                if resp.status_code == 400 and self.is_wrong_bid_value_error(response_text):
                    min_bid = self.extract_min_bid_from_error(response_text)
                    max_bid = self.extract_max_bid_from_error(response_text)

                    if min_bid is not None or max_bid is not None:
                        log(
                            f"🔁 advert_id={advert_payload['advert_id']} "
                            f"корректирую payload по ответу WB: min={min_bid}, max={max_bid}"
                        )
                        advert_payload = self.apply_error_constraints_to_single_advert(
                            advert_payload,
                            response_text,
                        )
                    else:
                        log(
                            f"🔁 advert_id={advert_payload['advert_id']} "
                            f"min/max не извлечены, повышаю на +{self.fallback_bid_step_kopecks} коп"
                        )
                        advert_payload = self.bump_single_advert_bids(advert_payload)

                    time.sleep(0.3)
                    continue

                if resp.status_code in self.WB_RETRYABLE_HTTP_CODES:
                    sleep_for = min(2 ** attempt, 20)
                    log(
                        f"🔁 advert_id={advert_payload['advert_id']} "
                        f"retryable HTTP {resp.status_code}, пауза {sleep_for} сек"
                    )
                    time.sleep(sleep_for)
                    continue
                ...
                log(f"⚠️ advert_id={advert_payload['advert_id']} исключение, попытка {attempt}: {e}")
                send_log_rows.append({
                    "Дата": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    "ID кампании": advert_payload["advert_id"],
                    "Попытка": attempt,
                ...
                if attempt < self.max_retry_rounds:
        ...
        log(
            f"⚠️ advert_id={advert_payload['advert_id']} не отправлен "
            f"после {self.max_retry_rounds} попыток"
        )
        log(
            f"⚠️ Финальный payload advert_id={advert_payload['advert_id']}: "
            f"{json.dumps({'bids': [advert_payload]}, ensure_ascii=False)[:4000]}"
        )

        send_log_rows.append({
            "Дата": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "ID кампании": advert_payload["advert_id"],
            "Попытка": self.max_retry_rounds,
            "Статус": "final_failed",
            "HTTP статус": "",
            "Ответ WB": json.dumps({"bids": [advert_payload]}, ensure_ascii=False)[:4000],
        })

        return False, pd.DataFrame(send_log_rows)
        ...
        if dry_run:
            ...
            for advert in adverts:
                send_log_rows.append({
                    ...
                    "Статус": "dry-run",
                    "HTTP статус": "",
                    "Ответ WB": json.dumps({"bids": [advert]}, ensure_ascii=False)[:2000],
                })
            return len(adverts), 0, pd.DataFrame(send_log_rows)

        success = 0
        failed = 0

        total = len(adverts)
        for idx, advert_payload in enumerate(adverts, start=1):
            advert_id = safe_int(advert_payload.get("advert_id"))
            log(f"📤 Отправка кампании {idx}/{total}: advert_id={advert_id}")

            ok, one_log_df = self.send_one_advert(advert_payload)
            if not one_log_df.empty:
                send_log_rows.extend(one_log_df.to_dict("records"))
