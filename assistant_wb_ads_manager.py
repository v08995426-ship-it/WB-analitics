...
# ===================== FINAL OVERRIDE 2026-04-28 ANTICRISIS / PRODUCT-BLOCK =====================
CATEGORY_DRR_LIMITS.update({
    'кисти косметические': 0.10,
    'косметические карандаши': 0.14,
    'помады': 0.16,
    'блески': 0.16,
})
OUT_PAUSE_HISTORY = SERVICE_ROOT + 'История_пауз_кампаний.xlsx'


class Config:
    category_drr_hard: float = 0.10
    category_drr_soft: float = 0.14
    campaign_drr_raise_max: float = 0.10
    product_drr_hard: float = 0.10
    product_drr_reduce_from: float = 0.15
    campaign_pause_drr: float = 0.18
    min_profitability: float = 0.05
    min_daily_impressions: float = 1000.0
    high_impressions_for_pause: float = 10000.0
    eval_days: int = 3
    pause_check_days: int = 5
    cpo_bad_vs_avg: float = 1.35
    cpo_bad_vs_best: float = 1.60


def get_product_block(subject_norm: Any, supplier_article: Any, product_root: Any) -> str:
    subject_norm = canonical_subject(subject_norm)
    supplier_article = str(supplier_article or '').strip()
    product_root = str(product_root or '').strip()
    if subject_norm == 'кисти косметические':
        return supplier_article
    return product_root or supplier_article


def build_product_block_metrics(rows: pd.DataFrame) -> pd.DataFrame:
    if rows is None or rows.empty:
        return pd.DataFrame(columns=['product_block'])
    df = rows.copy()
    df['product_block'] = df.apply(lambda r: get_product_block(r.get('subject_norm'), r.get('supplier_article'), r.get('product_root')), axis=1)
    df['campaign_margin_pct'] = np.where(pd.to_numeric(df.get('Расход'), errors='coerce').fillna(0.0) > 0,
                                         pd.to_numeric(df.get('campaign_profit_cur'), errors='coerce').fillna(0.0) /
                                         pd.to_numeric(df.get('Расход'), errors='coerce').fillna(0.0),
                                         0.0)
    grp = df.groupby(['subject_norm', 'product_block'], as_index=False).agg(
        block_campaigns=('id_campaign', 'nunique'),
        block_spend_cur=('Расход', 'sum'),
        block_revenue_cur=('Сумма_заказов', 'sum'),
        block_orders_cur=('Заказы', 'sum'),
        block_profit_cur=('campaign_profit_cur', 'sum'),
        block_spend_base=('base_Расход', 'sum'),
        block_orders_base=('base_Заказы', 'sum'),
        block_profit_base=('campaign_profit_base', 'sum'),
        block_avg_cpo=('campaign_cpo', 'mean'),
        block_best_cpo=('campaign_cpo', 'min'),
        block_best_margin=('campaign_margin_pct', 'max'),
        block_avg_margin=('campaign_margin_pct', 'mean'),
    )
    grp['block_drr_cur'] = np.where(grp['block_revenue_cur'] > 0, grp['block_spend_cur'] / grp['block_revenue_cur'], 0.0)
    ...
    grp['block_profit_growth_pct'] = np.where(grp['block_profit_base'] != 0, (grp['block_profit_cur'] - grp['block_profit_base']) / np.abs(grp['block_profit_base']) * 100.0, np.where(grp['block_profit_cur'] > 0, 100.0, 0.0))
    return grp


def load_internal_bid_history(provider: BaseProvider) -> pd.DataFrame:
    # История ставок теперь живёт в итоговом workbook, поэтому читаем её оттуда.
    candidates = [OUT_SINGLE_REPORT, OUT_PREVIEW]
    for key in candidates:
        try:
            if provider.file_exists(key):
                df = provider.read_excel(key, sheet_name='История ставок')
...
def build_campaign_control_signals(ads_daily: pd.DataFrame, keywords: pd.DataFrame, campaigns: pd.DataFrame, master: pd.DataFrame, as_of_date: date, bid_hist: pd.DataFrame) -> pd.DataFrame:
    base_cols = ['id_campaign', 'nmId', 'impressions_3d', 'clicks_3d', 'orders_3d', 'spend_3d', 'revenue_3d', 'impressions_prev3', 'clicks_prev3', 'orders_prev3', 'spend_prev3', 'revenue_prev3', 'position_3d', 'visibility_3d', 'demand_3d', 'capture_imp_3d', 'days_since_change', 'last_bid_delta', 'last_bid_rub']
    if ads_daily is None or ads_daily.empty:
        return pd.DataFrame(columns=base_cols)

    cur_end = as_of_date - timedelta(days=MATURE_END_OFFSET)
    last3_start = cur_end - timedelta(days=2)
    prev3_end = last3_start - timedelta(days=1)
    prev3_start = prev3_end - timedelta(days=2)

    ad3 = ads_daily[(ads_daily['date'] >= last3_start) & (ads_daily['date'] <= cur_end)].groupby(['id_campaign', 'nmId'], as_index=False).agg(
        impressions_3d=('Показы', 'sum'),
        clicks_3d=('Клики', 'sum'),
        orders_3d=('Заказы', 'sum'),
        spend_3d=('Расход', 'sum'),
        revenue_3d=('Сумма заказов', 'sum'),
    )
    ad_prev = ads_daily[(ads_daily['date'] >= prev3_start) & (ads_daily['date'] <= prev3_end)].groupby(['id_campaign', 'nmId'], as_index=False).agg(
        impressions_prev3=('Показы', 'sum'),
        clicks_prev3=('Клики', 'sum'),
        orders_prev3=('Заказы', 'sum'),
        spend_prev3=('Расход', 'sum'),
        revenue_prev3=('Сумма заказов', 'sum'),
    )

    out = ad3.merge(ad_prev, on=['id_campaign', 'nmId'], how='outer').fillna(0)

    if keywords is not None and not keywords.empty:
        kw = dedupe_keyword_rows(keywords)
        km = master[['nmId', 'supplier_article']].drop_duplicates() if master is not None and not master.empty else pd.DataFrame(columns=['nmId', 'supplier_article'])
        if not km.empty:
            kw = kw.merge(km, on=['nmId', 'supplier_article'], how='inner') if 'supplier_article' in kw.columns else kw.merge(km, on='nmId', how='left')
        kw3 = kw[(kw['date'] >= last3_start) & (kw['date'] <= cur_end)].groupby('nmId', as_index=False).agg(
            position_3d=('median_position', 'median'),
            visibility_3d=('visibility_pct', 'mean'),
            demand_3d=('demand_week', 'sum'),
        )
        out = out.merge(kw3, on='nmId', how='left')
    else:
        out['position_3d'] = 0.0
        out['visibility_3d'] = 0.0
        out['demand_3d'] = 0.0

    out['capture_imp_3d'] = np.where(pd.to_numeric(out.get('demand_3d'), errors='coerce').fillna(0.0) > 0,
                                     pd.to_numeric(out.get('impressions_3d'), errors='coerce').fillna(0.0) / pd.to_numeric(out.get('demand_3d'), errors='coerce').fillna(0.0),
                                     0.0)

    out['days_since_change'] = 999.0
    out['last_bid_delta'] = 0.0
    out['last_bid_rub'] = 0.0
    if bid_hist is not None and not bid_hist.empty:
        bh = bid_hist.copy()
        bh = bh.rename(columns={'ID кампании': 'id_campaign', 'Артикул WB': 'nmId', 'Новая ставка, ₽': 'bid_rub', 'Дата запуска': 'run_ts'})
        bh['id_campaign'] = pd.to_numeric(bh.get('id_campaign'), errors='coerce')
        bh['nmId'] = pd.to_numeric(bh.get('nmId'), errors='coerce')
        bh['bid_rub'] = pd.to_numeric(bh.get('bid_rub'), errors='coerce')
        bh['run_ts'] = pd.to_datetime(bh.get('run_ts'), errors='coerce')
        bh = bh.dropna(subset=['id_campaign', 'nmId', 'run_ts']).sort_values(['id_campaign', 'nmId', 'run_ts'])
        last_rows = []
        for (cid, nm), g in bh.groupby(['id_campaign', 'nmId']):
            g = g.tail(2)
            last = g.iloc[-1]
            prev = g.iloc[-2] if len(g) >= 2 else None
            delta = safe_float(last.get('bid_rub')) - (safe_float(prev.get('bid_rub')) if prev is not None else safe_float(last.get('bid_rub')))
            days_since = (as_of_date - last['run_ts'].date()).days if pd.notna(last['run_ts']) else 999
            last_rows.append({'id_campaign': int(cid), 'nmId': int(nm), 'days_since_change': days_since, 'last_bid_delta': delta, 'last_bid_rub': safe_float(last.get('bid_rub'))})
        if last_rows:
            out = out.merge(pd.DataFrame(last_rows), on=['id_campaign', 'nmId'], how='left', suffixes=('', '_hist'))
            out['days_since_change'] = pd.to_numeric(out.get('days_since_change_hist', out.get('days_since_change')), errors='coerce').fillna(out['days_since_change'])
            out['last_bid_delta'] = pd.to_numeric(out.get('last_bid_delta_hist', out.get('last_bid_delta')), errors='coerce').fillna(out['last_bid_delta'])
            out['last_bid_rub'] = pd.to_numeric(out.get('last_bid_rub_hist', out.get('last_bid_rub')), errors='coerce').fillna(out['last_bid_rub'])
            out = out.drop(columns=[c for c in ['days_since_change_hist', 'last_bid_delta_hist', 'last_bid_rub_hist'] if c in out.columns])
    return out
...
def determine_action(row: pd.Series, cfg: Config) -> Tuple[str, float, str, bool]:
    ...
    max_bid = safe_float(row.get('max_bid_rub'))
    has_max = max_bid > 0
    subject_norm = canonical_subject(row.get('subject_norm'))
    product_block = str(row.get('product_block') or row.get('supplier_article') or '')

    category_drr = safe_float(row.get('category_drr_cur'))
    category_profit_growth = safe_float(row.get('category_gp_growth_pct'))
    category_orders_growth = safe_float(row.get('category_orders_growth_pct'))
    category_demand_growth = safe_float(row.get('category_demand_growth_pct'))

    block_drr = safe_float(row.get('block_drr_cur'))
    block_profit_growth = safe_float(row.get('block_profit_growth_pct'))
    block_orders_growth = safe_float(row.get('block_orders_growth_pct'))
    block_best_cpo = max(0.0, safe_float(row.get('block_best_cpo')))
    block_avg_cpo = max(0.0, safe_float(row.get('block_avg_cpo')))
    block_campaigns = safe_int(row.get('block_campaigns'))

    campaign_drr = safe_float(row.get('campaign_drr_cur'))
    campaign_profit_cur = safe_float(row.get('campaign_profit_cur'))
    campaign_profit_growth = safe_float(row.get('campaign_profit_growth_pct'))
    campaign_orders_growth = safe_float(row.get('campaign_order_growth_pct'))
    campaign_impr_growth = safe_float(row.get('campaign_impression_growth_pct'))
    campaign_click_growth = safe_float(row.get('campaign_click_growth_pct'))
    campaign_cpo = safe_float(row.get('campaign_cpo'))
    campaign_margin = safe_float(row.get('campaign_margin_pct'))
    visibility_3d = safe_float(row.get('visibility_3d'))
    position_3d = safe_float(row.get('position_3d'))
    impressions_3d = safe_float(row.get('impressions_3d'))
    days_since_change = safe_float(row.get('days_since_change', 999))
    last_bid_delta = safe_float(row.get('last_bid_delta'))
    capture_imp_3d = safe_float(row.get('capture_imp_3d'))
    capture_imp_prev3 = safe_float(row.get('capture_imp_prev3'))

    can_raise = has_max and current_bid + step <= max_bid + 1e-9
    can_lower = current_bid - step >= floor_bid - 1e-9

    def up() -> float:
        if not can_raise:
            return round(current_bid, 2)
        return round(min(current_bid + step, max_bid), 2)

    def down() -> float:
        return round(max(current_bid - step, floor_bid), 2)

    level1 = f"Уровень 1 Категория: {get_subject_display_name(subject_norm)}; ДРР {category_drr*100:.1f}%, рост заказов {category_orders_growth:.1f}%, рост ВП {category_profit_growth:.1f}%, рост спроса {category_demand_growth:.1f}%"
    level2 = f"Уровень 2 Товарный блок: {product_block}; ДРР блока {block_drr*100:.1f}%, CPO лучший {block_best_cpo:.0f} ₽, CPO средний {block_avg_cpo:.0f} ₽, рост заказов {block_orders_growth:.1f}%, рост ВП {block_profit_growth:.1f}%"
    level3 = f"Уровень 3 Кампания: ДРР {campaign_drr*100:.1f}%, CPO {campaign_cpo:.0f} ₽, рентабельность {campaign_margin*100:.1f}%, рост показов {campaign_impr_growth:.1f}%, кликов {campaign_click_growth:.1f}%, заказов {campaign_orders_growth:.1f}%, ВП {campaign_profit_growth:.1f}%"

    # Без max bid кампанию нельзя повышать.
    if not has_max and current_bid > 0:
        if campaign_drr > max(cfg.campaign_drr_raise_max, cfg.category_drr_hard) and can_lower:
            return 'Снизить', down(), f"{level1}; {level2}; {level3}; максимальная ставка не рассчитана, при ДРР выше порога сушим", False
        return 'Без изменений', round(current_bid, 2), f"{level1}; {level2}; {level3}; максимальная ставка не рассчитана — рост запрещён", False

    # Контроль после повышения: за 3 дня ставка выросла, а ключевые сигналы не выросли.
    if 0 <= days_since_change <= cfg.eval_days and last_bid_delta > 0:
        no_signal = (campaign_impr_growth < 5) and (campaign_click_growth < 5) and (campaign_orders_growth <= 0) and (campaign_profit_growth <= 0)
        if no_signal and can_lower:
            return 'Снизить', down(), f"{level1}; {level2}; {level3}; недавнее повышение не дало роста показов/кликов/заказов/ВП — возвращаем шаг", False

    # Контроль после снижения: уронили ставку и потеряли заказы/ВП.
    if 0 <= days_since_change <= cfg.eval_days and last_bid_delta < 0:
        if campaign_orders_growth < 0 and campaign_profit_growth < 0 and can_raise:
            return 'Повысить', up(), f"{level1}; {level2}; {level3}; после недавнего снижения просели заказы и ВП — возвращаем шаг", False

    # Антикризис: если ДРР категории >10%, перераспределяем трафик внутри категории и товарного блока.
    if category_drr > cfg.category_drr_hard or block_drr > cfg.product_drr_hard:
        if campaign_drr > cfg.campaign_drr_raise_max:
            if can_lower:
                return 'Снизить', down(), f"{level1}; {level2}; {level3}; антикризис: ДРР категории/товара выше 10%, кампанию с ДРР выше 10% сушим", False
            return 'Без изменений', round(current_bid, 2), f"{level1}; {level2}; {level3}; антикризис: ДРР высокий, но ставка уже у нижней границы", False
        if campaign_drr < cfg.campaign_drr_raise_max and campaign_cpo > 0 and (block_best_cpo == 0 or campaign_cpo <= block_best_cpo * 1.15) and campaign_margin >= cfg.min_profitability and can_raise:
            return 'Повысить', up(), f"{level1}; {level2}; {level3}; антикризис: переливаем трафик в самую дешёвую и доходную кампанию", False
        return 'Без изменений', round(current_bid, 2), f"{level1}; {level2}; {level3}; антикризис: кампанию не трогаем, она не является явным донором или реципиентом трафика", False

    # Кампания мало работает: пробуем раскачать до минимума трафика.
    if impressions_3d < cfg.min_daily_impressions * cfg.eval_days:
        if can_raise:
            return 'Повысить', up(), f"{level1}; {level2}; {level3}; меньше {cfg.min_daily_impressions:.0f} показов в день за 3 дня — аккуратно разгоняем трафик", False
        return 'Без изменений', round(current_bid, 2), f"{level1}; {level2}; {level3}; трафика мало, но выше max идти нельзя", False

    # Если 3 дня ставку не трогали и мы топ-10 / видимость >90%, пробуем сушку ради экономии.
    if days_since_change >= cfg.eval_days and position_3d > 0 and position_3d <= 10 and visibility_3d >= 90 and can_lower:
        return 'Снизить', down(), f"{level1}; {level2}; {level3}; 3 дня без изменений, позиция топ-10 и видимость >90% — тестируем снижение ставки", False

    # Рост разрешаем только если он дешёвый и прибыльный.
    if campaign_drr < cfg.campaign_drr_raise_max and campaign_margin >= cfg.min_profitability and campaign_cpo > 0:
        if block_best_cpo == 0 or campaign_cpo <= block_best_cpo * 1.10:
            if (campaign_impr_growth > 0 or campaign_click_growth > 0 or campaign_orders_growth > 0 or campaign_profit_growth > 0) and can_raise:
                return 'Повысить', up(), f"{level1}; {level2}; {level3}; кампания дешёвая и прибыльная, сигнал роста подтверждён", False

    # Дорогой трафик сушим.
    if campaign_drr > cfg.product_drr_reduce_from or (block_avg_cpo > 0 and campaign_cpo > block_avg_cpo * cfg.cpo_bad_vs_avg):
        if can_lower:
            return 'Снизить', down(), f"{level1}; {level2}; {level3}; кампания дороже среднего по товару — сушим", False
...
def prepare_metrics(provider: BaseProvider, cfg: Config, as_of_date: date) -> Dict[str, Any]:
    ...
    keywords = load_keywords(provider)
    globals()['keywords_global_for_history'] = keywords.copy()
    bid_history = load_internal_bid_history(provider)
    master = build_master(econ, orders, keywords, campaigns)
    log(f"📣 Реклама: {len(ads_daily):,} строк; кампании: {campaigns['id_campaign'].nunique() if not campaigns.empty else 0}; placement-строк: {len(campaigns):,}")
    plan_df, category_plan = build_previous_month_plan(orders, funnel, ads_daily, keywords, econ, as_of_date, master)
    log(f"💰 Экономика: {len(econ):,} SKU; Заказы: {len(orders):,} строк; Воронка: {len(funnel):,}; Keywords: {len(keywords):,}; План строк: {0 if 'Комментарий' in plan_df.columns else len(plan_df):,}")
    ...
    rows['subject'] = series_or_default(rows, 'subject', series_or_default(rows, 'subject_norm', '')).fillna('').astype(str)
    rows['subject_norm'] = series_or_default(rows, 'subject_norm', rows['subject'].map(canonical_subject)).fillna('').astype(str).map(canonical_subject)
    rows['product_root'] = series_or_default(rows, 'product_root', rows['supplier_article'].map(product_root_from_supplier_article)).fillna('').astype(str)
    rows['campaign_status'] = series_or_default(rows, 'campaign_status', '').fillna('').astype(str)
    rows['campaign_is_active'] = rows['campaign_status'].map(is_active_campaign_status)
    rows['product_block'] = rows.apply(lambda r: get_product_block(r.get('subject_norm'), r.get('supplier_article'), r.get('product_root')), axis=1)
    ...
    rows['ctr_pct'] = np.where(rows['Показы'] > 0, rows['Клики'] / rows['Показы'] * 100.0, 0.0)
    rows['campaign_drr_cur'] = np.where(rows['Сумма_заказов'] > 0, rows['Расход'] / rows['Сумма_заказов'], 0.0)
    rows['campaign_drr_base'] = np.where(rows['base_Сумма_заказов'] > 0, rows['base_Расход'] / rows['base_Сумма_заказов'], 0.0)
    rows['campaign_gp_cur'] = rows['Заказы'] * rows['gp_realized'] - rows['Расход']
    rows['campaign_gp_base'] = rows['base_Заказы'] * rows['gp_realized'] - rows['base_Расход']
    rows['campaign_profit_cur'] = rows['Заказы'] * rows['buyout_rate'] * rows['np_unit'] - rows['Расход']
    rows['campaign_profit_base'] = rows['base_Заказы'] * rows['buyout_rate'] * rows['np_unit'] - rows['base_Расход']
    rows['campaign_profit_growth_pct'] = np.where(rows['campaign_profit_base'] != 0, (rows['campaign_profit_cur'] - rows['campaign_profit_base']) / np.abs(rows['campaign_profit_base']) * 100.0, np.where(rows['campaign_profit_cur'] > 0, 100.0, 0.0))
    rows['campaign_gp_growth_pct'] = np.where(rows['campaign_gp_base'] != 0, (rows['campaign_gp_cur'] - rows['campaign_gp_base']) / np.abs(rows['campaign_gp_base']) * 100.0, np.where(rows['campaign_gp_cur'] > 0, 100.0, 0.0))
    rows['campaign_order_growth_pct'] = np.where(rows['base_Заказы'] > 0, (rows['Заказы'] / rows['base_Заказы'] - 1) * 100.0, np.where(rows['Заказы'] > 0, 100.0, 0.0))
    rows['campaign_click_growth_pct'] = np.where(rows['base_Клики'] > 0, (rows['Клики'] / rows['base_Клики'] - 1) * 100.0, np.where(rows['Клики'] > 0, 100.0, 0.0))
    rows['campaign_impression_growth_pct'] = np.where(rows['base_Показы'] > 0, (rows['Показы'] / rows['base_Показы'] - 1) * 100.0, np.where(rows['Показы'] > 0, 100.0, 0.0))
    rows['campaign_cpo'] = np.where(rows['Заказы'] > 0, rows['Расход'] / rows['Заказы'], 0.0)
    rows['campaign_roi_cur'] = np.where(rows['Расход'] > 0, (rows['campaign_profit_cur']) / rows['Расход'], 0.0)
    rows['campaign_margin_pct'] = np.where(rows['Расход'] > 0, rows['campaign_profit_cur'] / rows['Расход'], 0.0)
    ...
    item_order_growth_pct = numeric_series(rows, 'item_order_growth_pct', 0.0)
    base_item_spend_cur = numeric_series(rows, 'base_item_spend_cur', 0.0)
    rows['blended_drr'] = np.where(item_revenue_cur > 0, item_spend_cur / item_revenue_cur, 0.0)
    rows['total_orders'] = item_orders_cur
    rows['total_revenue'] = item_revenue_cur
    rows['ad_spend'] = item_spend_cur
    rows['ad_orders'] = numeric_series(rows, 'Заказы', 0.0)
    rows['order_growth_pct'] = item_order_growth_pct
    rows['spend_growth_pct'] = np.where(base_item_spend_cur > 0, (item_spend_cur / base_item_spend_cur.replace(0, np.nan) - 1.0) * 100.0, 0.0)
    rows['required_growth_pct'] = rows.apply(lambda r: compute_required_growth(safe_float(r.get('campaign_drr_cur')), safe_float(r.get('spend_growth_pct')), str(r.get('subject_norm',''))), axis=1)
    rows['card_issue'] = False
    rows['item_gp_growth_pct'] = numeric_series(rows, 'item_gp_growth_pct', 0.0)

    if category_plan is not None and not category_plan.empty and 'subject_norm' in category_plan.columns:
        cat_cols = [c for c in ['subject_norm','Факт ВП MTD, ₽','План ВП MTD, ₽','Темп плана ВП, %','Лимит ДРР категории, доля'] if c in category_plan.columns]
        cat_merge = category_plan[cat_cols].drop_duplicates('subject_norm').copy().rename(columns={
            'Факт ВП MTD, ₽': 'category_gp_mtd',
            'План ВП MTD, ₽': 'category_gp_plan',
            'Темп плана ВП, %': 'category_plan_attainment_pct',
            'Лимит ДРР категории, доля': 'category_limit_drr_plan',
    ...
    rows['category_limit_drr'] = numeric_series(rows, 'category_limit_drr', np.nan).fillna(rows['subject_norm'].map(get_category_drr_limit))
    rows['category_plan_attainment_pct'] = numeric_series(rows, 'category_plan_attainment_pct', 100.0)
    ...
    rows = rows.merge(build_channel_balance(ads_daily, campaigns, master, econ_latest, window, funnel), on='supplier_article', how='left')
    product_blocks = build_product_block_metrics(rows)
    rows = rows.merge(product_blocks, on=['subject_norm', 'product_block'], how='left')

    # ранжирование кампаний внутри блока: кому давать трафик в первую очередь
    rank_src = rows[['subject_norm', 'product_block', 'id_campaign', 'campaign_cpo', 'campaign_profit_cur', 'campaign_drr_cur', 'Заказы']].copy()
    rank_src['campaign_cpo_rank'] = rank_src.groupby(['subject_norm', 'product_block'])['campaign_cpo'].rank(method='dense', ascending=True)
    rank_src['profit_rank'] = rank_src.groupby(['subject_norm', 'product_block'])['campaign_profit_cur'].rank(method='dense', ascending=False)
    rank_src['orders_rank'] = rank_src.groupby(['subject_norm', 'product_block'])['Заказы'].rank(method='dense', ascending=False)
    rank_src['block_score'] = rank_src['campaign_cpo_rank'].fillna(999) + rank_src['profit_rank'].fillna(999) + rank_src['orders_rank'].fillna(999)
    rank_src['block_rank'] = rank_src.groupby(['subject_norm', 'product_block'])['block_score'].rank(method='dense', ascending=True)
    rows = rows.merge(rank_src[['subject_norm', 'product_block', 'id_campaign', 'block_score', 'block_rank']], on=['subject_norm', 'product_block', 'id_campaign'], how='left')

    signals = build_campaign_control_signals(ads_daily, keywords, campaigns, master, as_of_date, bid_history)
    rows = rows.merge(signals, on=['id_campaign', 'nmId'], how='left')
    for c in ['impressions_3d','clicks_3d','orders_3d','spend_3d','revenue_3d','impressions_prev3','clicks_prev3','orders_prev3','spend_prev3','revenue_prev3','position_3d','visibility_3d','demand_3d','capture_imp_3d','days_since_change','last_bid_delta','last_bid_rub']:
        rows[c] = numeric_series(rows, c, 0.0)

    # Изменение доли трафика после последнего шага.
    rows['capture_imp_prev3'] = np.where(rows['demand_3d'] > 0, rows['impressions_prev3'] / rows['demand_3d'], 0.0)
    ...
    for _, r in rows.iterrows():
        ...
        status_rec = 'Оставить'
        if canonical_subject(r.get('subject_norm')) != 'кисти косметические' and safe_int(r.get('block_campaigns')) > 1 and safe_float(r.get('block_rank')) > 1:
            if safe_float(r.get('impressions_3d')) >= cfg.high_impressions_for_pause and safe_float(r.get('campaign_drr_cur')) >= cfg.campaign_pause_drr and safe_float(r.get('campaign_margin_pct')) < cfg.min_profitability:
                avg_cpo = max(1.0, safe_float(r.get('block_avg_cpo')))
                best_cpo = max(1.0, safe_float(r.get('block_best_cpo')))
                if safe_float(r.get('campaign_cpo')) >= avg_cpo * cfg.cpo_bad_vs_avg and safe_float(r.get('campaign_cpo')) >= best_cpo * cfg.cpo_bad_vs_best:
                    status_rec = 'Пауза рекомендована'
        if action in {'Повысить', 'Снизить'} and round(safe_float(new_bid), 2) == round(safe_float(r.get('current_bid_rub')), 2):
        ...
        decisions.append({
            ...
            'Артикул продавца': r.get('supplier_article',''),
            'Товар': r.get('supplier_article',''),
            'Товарный блок': r.get('product_block',''),
            'Предмет': r.get('subject',''),
            'Плейсмент': r.get('placement',''),
            'Тип кампании': f"{r.get('payment_type','')}_{r.get('placement','')}",
            'Текущая ставка, ₽': round(safe_float(r.get('current_bid_rub')), 2),
            ...
            'Действие': action,
            'Новая ставка, ₽': round(safe_float(new_bid), 2),
            'Рекомендация по статусу': status_rec,
            'Причина': reason,
            'Показы': round(safe_float(r.get('Показы')), 0),
            'Клики': round(safe_float(r.get('Клики')), 0),
            'CTR, %': round(safe_float(r.get('ctr_pct')), 2),
            ...
            'ДРР кампании, %': round(safe_float(r.get('campaign_drr_cur')) * 100, 2),
            'ВП кампании текущее окно после рекламы, ₽': round(safe_float(r.get('campaign_gp_cur')), 2),
            'Чистая прибыль кампании текущее окно, ₽': round(safe_float(r.get('campaign_profit_cur')), 2),
            'Рентабельность кампании, %': round(safe_float(r.get('campaign_margin_pct')) * 100, 2),
            'ВП кампании базовое окно после рекламы, ₽': round(safe_float(r.get('campaign_gp_base')), 2),
            'Рост ВП кампании, %': round(safe_float(r.get('campaign_profit_growth_pct')), 2),
            'ROI кампании, %': round(safe_float(r.get('campaign_roi_cur')) * 100, 2),
            'CPO кампании, ₽': round(safe_float(r.get('campaign_cpo')), 2),
            'Все заказы товара': round(safe_float(r.get('total_orders')), 2),
            'Выручка товара, ₽': round(safe_float(r.get('total_revenue')), 2),
            ...
            'Рост ВП блока, %': round(safe_float(r.get('block_profit_growth_pct')), 2),
            'CPO блока средний, ₽': round(safe_float(r.get('block_avg_cpo')), 2),
            'CPO блока лучший, ₽': round(safe_float(r.get('block_best_cpo')), 2),
            'Ранг в блоке': safe_int(r.get('block_rank')),
            'Показы 3д': round(safe_float(r.get('impressions_3d')), 0),
            'Клики 3д': round(safe_float(r.get('clicks_3d')), 0),
            'Заказы 3д': round(safe_float(r.get('orders_3d')), 2),
            'Позиция 3д': round(safe_float(r.get('position_3d')), 2),
            'Видимость 3д, %': round(safe_float(r.get('visibility_3d')), 2),
            'Дней с последней смены ставки': round(safe_float(r.get('days_since_change')), 0),
            'Последняя дельта ставки, ₽': round(safe_float(r.get('last_bid_delta')), 2),
            'Индекс эффективности ставки по показам': round(safe_float(r.get('eff_index_imp')), 4),
            'Индекс эффективности ставки по кликам': round(safe_float(r.get('eff_index_click')), 4),
        })
        if status_rec in {'Пауза рекомендована', 'Запуск рекомендован'}:
            pause_rows.append({
                'Дата запуска': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                'ID кампании': safe_int(r.get('id_campaign')),
                'Артикул продавца': r.get('supplier_article',''),
    ...
    decisions_df = pd.DataFrame(decisions)
    weak = decisions_df[(decisions_df['Действие'].isin(['Снизить'])) | (pd.to_numeric(decisions_df.get('Позиция 3д', 0), errors='coerce').fillna(0) > 20)].copy()
    weak['Комментарий'] = weak['Причина']
    weak = weak[[c for c in ['Артикул продавца','Артикул WB','ID кампании','Тип кампании','Плейсмент','Действие','Рекомендация по статусу','Комментарий'] if c in weak.columns]].drop_duplicates()

    product_metrics = rows.groupby(['product_block','subject_norm'], as_index=False).agg(
        total_orders=('total_orders','max'),
        total_revenue=('total_revenue','max'),
        ad_spend=('ad_spend','max'),
        blended_drr=('blended_drr','max'),
        item_gp_growth_pct=('item_gp_growth_pct','max'),
        block_campaigns=('block_campaigns','max'),
        block_drr_cur=('block_drr_cur','max'),
        block_orders_growth_pct=('block_orders_growth_pct','max'),
        block_profit_growth_pct=('block_profit_growth_pct','max'),
        block_avg_cpo=('block_avg_cpo','max'),
        block_best_cpo=('block_best_cpo','max'),
    ).rename(columns={'product_block':'Товарный блок','subject_norm':'Предмет код'})
    product_metrics['Общий ДРР товара, %'] = (product_metrics['blended_drr'] * 100).round(2)

    campaign_profit = decisions_df[[c for c in ['Артикул продавца','Товарный блок','ID кампании','Тип кампании','ДРР кампании, %','ВП кампании текущее окно после рекламы, ₽','Чистая прибыль кампании текущее окно, ₽','Рентабельность кампании, %','CPO кампании, ₽','Ранг в блоке','Рекомендация по статусу','Причина'] if c in decisions_df.columns]].copy()
    effects = decisions_df[decisions_df['Действие'].isin(['Повысить','Снизить']) & (decisions_df['Новая ставка, ₽'] != decisions_df['Текущая ставка, ₽'])].copy()
    if effects.empty:
        effects = pd.DataFrame([{'Комментарий':'В этом запуске не было изменений ставок'}])
    ...
    return {
        ...
        'effects': effects,
        'shade_portfolio': pd.DataFrame([{'Комментарий':'Оттенки отключены'}]),
        'shade_actions': pd.DataFrame([{'Комментарий':'Оттенки отключены'}]),
        'shade_tests': pd.DataFrame([{'Комментарий':'Оттенки отключены'}]),
        'eff_history_sheets': build_efficiency_history(ads_daily, campaigns, keywords_current if False else aggregate_keyword_daily(keywords), master, load_bid_history(provider), as_of_date),
        'window': window,
        'daily_item_history': daily_item,
        'daily_campaign_history': daily_campaign,
        'plan_sheet': plan_df,
        'category_plan': category_plan,
        'campaign_profit': campaign_profit,
        'pause_history_append': pd.DataFrame(pause_rows),
    }
...
def save_outputs(provider: BaseProvider, results: Dict[str, Any], run_mode: str, bid_send_log: Optional[pd.DataFrame], shade_apply_log: Optional[pd.DataFrame] = None, history_append: Optional[pd.DataFrame] = None) -> None:
    ...
    min_bids_df = results.get('min_bids_df', pd.DataFrame()).copy()
    history_append = history_append.copy() if isinstance(history_append, pd.DataFrame) else pd.DataFrame()
    pause_append = results.get('pause_history_append', pd.DataFrame()).copy() if isinstance(results.get('pause_history_append'), pd.DataFrame) else pd.DataFrame()
    ...
    summary = pd.DataFrame([{
        ...
        'Изменённых ставок': int(len(decisions[decisions['Действие'].isin(['Повысить', 'Снизить']) & (decisions['Новая ставка, ₽'] != decisions['Текущая ставка, ₽'])])) if not decisions.empty else 0,
        'Блоков отправки ставок': 0 if bid_send_log is None or bid_send_log.empty else int(len(bid_send_log)),
        'Пауза рекомендована': int((decisions.get('Рекомендация по статусу', pd.Series('', index=decisions.index)).astype(str) == 'Пауза рекомендована').sum()) if not decisions.empty else 0,
        'Текущее окно с': results['window']['cur_start'],
        'Текущее окно по': results['window']['cur_end'],
        'База с': results['window']['base_start'],
        'База по': results['window']['base_end'],
    ...
    item_hist_all = append_dedup(old_sheets.get('История день товар', pd.DataFrame()), daily_item, ['Дата', 'Артикул WB'], item_cols)
    campaign_hist_all = append_dedup(old_sheets.get('История день кампания', pd.DataFrame()), daily_campaign, ['Дата', 'ID кампании', 'Артикул WB'], campaign_cols)
    bid_hist_all = append_dedup(old_sheets.get('История ставок', pd.DataFrame()), decisions[decisions['Действие'].isin(['Повысить', 'Снизить']) & (decisions['Новая ставка, ₽'] != decisions['Текущая ставка, ₽'])].copy(), ['ID кампании', 'Артикул WB', 'Плейсмент', 'Дата запуска'])
    archive_all = append_dedup(old_sheets.get('Архив решений', pd.DataFrame()), decisions, ['Дата запуска', 'ID кампании', 'Артикул WB', 'Плейсмент'])
    pause_hist_all = append_dedup(old_sheets.get('История пауз', pd.DataFrame()), pause_append, ['Дата запуска', 'ID кампании', 'Рекомендация'])
    ...
    sheets = {
        ...
        'История ставок': bid_hist_all,
        'Архив решений': archive_all,
        'История пауз': pause_hist_all if not pause_hist_all.empty else pd.DataFrame([{'Комментарий': 'Паузы ещё не рекомендовали'}]),
        'Эффективность ставки': eff_stub,
        'История день товар': item_hist_all,
        'История день кампания': campaign_hist_all,
    }
